...
def main():
    ...
    for ticker in tqdm(tickers.get_us_tickers()):
        ...
        for metric in metrics.get_solvency_leverage_metrics():
            if metrics_df.empty:
                metrics_df = calculator.get_company_valuation_metric(ticker, metric, start_date, end_date, frequency='quarterly')
            else:
                metrics_df = pd.merge(metrics_df,
                                      calculator.get_company_valuation_metric(ticker, metric, start_date, end_date, frequency='quarterly'),
                                      on='date',
                                      how='outer')
